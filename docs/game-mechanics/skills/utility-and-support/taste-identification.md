![Taste Identification](../../assets/Flag_taste_identification.gif){ align=right }

# Taste Identification

Identify poisoned food and detect poison.

## Overview

Taste Identification allows you to detect poison in food and beverages before consuming them. Also provides some resistance to poison attempts. Situational skill with limited applications.

## Effects

### Poison Detection

- Detect poisoned food
- Identify poison in drinks
- Check food safety
- Prevent poisoning

## Training

### Active Training

Use Taste Identification on:

- Food items
- Potions
- Any consumable
- Repeated attempts

### Passive Training

- May gain when poisoned
- Slow passive gains
- Active training faster

## Related Skills

**Poison Skills:**

- [Poisoning](poisoning.md) - Create poisons
- [Resisting Spells](resisting-spells.md) - Resist effects
- [Alchemy](alchemy.md) - Make potions

## Uses

### Poison Protection

- Detect poisoned food
- Avoid assassination attempts
- Check suspicious items
- Safety precaution

### Limited Applications

- Very niche skill
- Rarely essential
- Situational use only
- Better skills available

## Importance

⭐ (Very niche, rarely used)

!!! note "Limited Usefulness"
    This skill has very limited practical applications. Most players skip it entirely in favor of more useful skills.

---

**Related Pages:**

- [Poisoning](poisoning.md)
- [Alchemy](alchemy.md)
- [Resisting Spells](resisting-spells.md)
